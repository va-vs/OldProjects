	...
	<script type="text/javascript">
		...
		function IsValid() {
			...
			if (!pass.getAttribute("readonly")) {
				//if (pass.value.length < 8||pwd.value.length>16) {
				//	spanPwd.innerHTML = "密码长度不能小于8位,且不能大于16位！";
				//	pass.select();
				//	return false;
				//}
                var testPassword =/^(?![a-zA-Z]+$)(?![A-Z0-9]+$)(?![A-Z\W_!@#$%^&*`~()-+=]+$)(?![a-z0-9]+$)(?![a-z\W_!@#$%^&*`~()-+=]+$)(?![0-9\W_!@#$%^&*`~()-+=]+$)[a-zA-Z0-9\W_!@#$%^&*`~()-+=]{8,15}$/;
                if (testPassword.test(pwd.value) == false) {
                    spanPwd.innerHTML ="您的密码复杂度太低（密码中只能是大写字母、小写字母、数字、下划线中至少三种组合而成，且长度为8-16位之间），请重新输入密码！";
                    pwd.select();
                    return false;
                }
	...
	<div id="divregInfo" runat="server" visible="true">
		...
		<table style="font-size: 14px; color: #494b4c" cellspacing="0" cellpadding="3">
			...
			<tr>
				...
				<td>
					<asp:TextBox ID="txtPwd" runat="server" TextMode="Password" CssClass="txtcss" onfocus="onfocusjs()" onblur="testpwd()" ToolTip="必填项！密码最少6位" CausesValidation="True" AutoPostBack="false"></asp:TextBox><span style="font-size: 14px; color: red; margin-left: 10px" id="spanPwd"></span><br />
	...
	<div id="divregSuccess" runat="server" visible="false">
		<table cellpadding="0" align="center" class="stable">
			<tbody>
				<tr>
					<th valign="middle">
						<span style="color: rgb(255, 255, 255); font-size: 20px; font-family: 微软雅黑;">
							<asp:Label ID="lb0" runat="server" Text="Label"></asp:Label>
							辽宁省研究生数学建模竞赛
						</span>
					</th>
				</tr>
					...
					<td>
						<div style="padding: 25px 35px 40px; background-color: #F3F3F3;">
							...
							<div style="line-height: 25px; font-size: 14px;">
								您好，欢迎您报名参加<asp:Label ID="lb1" runat="server" Text="Label"></asp:Label>辽宁省研究生数学建模竞赛！<br />
